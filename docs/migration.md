...
The current generator emits revision 7 so generated parser rules can use the
neutral-parse transaction and resume contract required by opt-in shared
descent. This runtime continues to accept revisions 1 through 6 because their
required APIs remain supported.
