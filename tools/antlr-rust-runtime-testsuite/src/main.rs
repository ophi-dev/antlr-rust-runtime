...
const RENDER_DRIVER: &str = "tools/antlr-rust-runtime-testsuite/java/RenderGrammar.java";
const RUST_TEST_STG: &str = "tools/antlr-rust-runtime-testsuite/templates/Rust.test.stg";
    ...
    fn prepare(args: &'a Args) -> io::Result<Self> {
        ...
        run_checked(
            Command::new("javac")
                ...
                .arg("-d")
                .arg(&render_classes)
                .arg(args.runtime_crate.join(RENDER_DRIVER)),
...
impl Args {
    fn parse() -> Result<Self, String> {
        ...
        let mut runtime_crate = Path::new(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .nth(2)
        ...
        let antlr_jar = resolve_path_argument(
            antlr_jar,
            ANTLR_JAR_ENV,
            runtime_crate.join(DEFAULT_ANTLR_JAR),
            "--antlr-jar",
            "ANTLR tool jar",
        )?;
        let descriptors = resolve_path_argument(
            descriptors,
            DESCRIPTORS_ENV,
            runtime_crate.join(DEFAULT_DESCRIPTORS),
            "--descriptors",
        ...
        let stg = stg.unwrap_or_else(|| runtime_crate.join(RUST_TEST_STG));
...
fn usage() -> String {
    format!(
        "usage: antlr4-runtime-testsuite [--antlr-jar ANTLR.jar] [--descriptors PATH] [--case Group/Name] [--group Group] [--limit N] [--jobs N] [--keep] [--stg PATH]\n\nDefaults: ANTLR4_JAR or target/antlr-cleanroom/tools/antlr-4.13.2-complete.jar; ANTLR4_RUNTIME_TESTSUITE or target/antlr-cleanroom/antlr4-upstream/runtime-testsuite; --stg {RUST_TEST_STG}; --jobs min(cores, 8)"
    )
}
