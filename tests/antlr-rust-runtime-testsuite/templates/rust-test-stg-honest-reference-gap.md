...
> StringTemplate engine (`../java/RenderGrammar.java` via the
...
> translation (`crates/antlr-rust-codegen/src/embedded/` — the Rust analog of ANTLR's
...
| Rule return attributes as public fields (`ctx.v`) | Values kept in a runtime `int_return` map keyed by rule-index (`crates/antlr-rust-runtime/src/tree.rs`) |
