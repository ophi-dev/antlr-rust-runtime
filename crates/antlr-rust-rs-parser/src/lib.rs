//! Checked-in Rust recognizers and syntax analysis for embedded-action lowering.
//!
//! The recognizers are generated by `antlr-rust-codegen` itself. The analysis
//! facade validates each completed parse and builds its role and scope indexes
//! through the generated typed listener.
...
use antlr4_runtime::{
    AsRuleNode, CommonTokenStream, ErrorListener, InputStream, Node, Parser as _, Recognizer,
    SyntaxErrorEvent, TokenId, TokenStore,
};
...
use generated::parser::{
    AssociatedConstDeclContext, AssociatedStaticDeclContext, AttrContext, ConstDeclContext,
    EnumDeclContext, EnumVariantMainContext, ExternCrateContext, FieldContext, FnDeclContext,
    FnHeadContext, ForeignFnDeclContext, ForeignItemTailContext, ImplItemTailContext,
    InnerAttrContext, LifetimeContext, MacroDeclContext, MacroInvocationContext,
    MacroInvocationSemiContext, MacroRulesDefinitionContext, MacroTailContext, MethodDeclContext,
    ModDeclContext, ModDeclShortContext, PatFieldContext, PrimExprNoStructContext, RULE_ANY_IDENT,
    RULE_ATTR, RULE_BLOCK, RULE_BLOCK_WITH_INNER_ATTRS, RULE_CLOSURE_PARAM, RULE_CLOSURE_PARAMS,
...
fn analyze_with_root(body: &str, analysis_root: AnalysisRoot) -> io::Result<RustSyntax> {
    ...
    let validated = RustParserParseOutput {
        result: root,
        parser,
    }
    .validate()
    .map_err(|error| rust_syntax_error(&error.to_string()))?;
    let parsed = validated.parsed_file();

    // Invocation classification depends on all lexical macro and path shadows.
    let mut binding_collector = BindingCollector::new(parsed.tokens(), body);
    binding_collector.walk(validated.tree())?;
    let bindings = binding_collector.finish();

    let mut analyzer = SyntaxAnalyzer::new(parsed.tokens(), body, analysis_root, &bindings);
    analyzer.walk(validated.tree())?;
    Ok(analyzer.finish())
}

#[derive(Debug, Default)]
struct BindingIndex {
    macros: Vec<ScopedMacroBinding>,
    standard_paths: Vec<ScopedStandardPathBinding>,
}
...
impl<'a> BindingCollector<'a> {
    const fn new(tokens: &'a TokenStore, body: &'a str) -> Self {
        Self {
            tokens,
            body,
            body_len: body.len(),
    ...
    fn collect_macro_definition(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        let Some(name) = macro_rules_name(rule, self.tokens) else {
            return;
        };
        let Some(declaration) = body_byte_range(rule, self.tokens, self.body_len) else {
            return;
        };
        ...
        self.bindings.macros.push(ScopedMacroBinding {
            name: name.clone(),
            scope,
            activation: activation.clone(),
        });
        self.bindings.macros.extend(collect_macro_use_bindings(
            rule,
            &name,
            activation,
            self.tokens,
    ...
    fn collect_standard_paths(
        &mut self,
        rule: antlr4_runtime::RuleNodeView<'_>,
        introduced: Vec<ImportedPathBinding>,
    ) {
        let scope = enclosing_block(rule)
            .and_then(|block| body_byte_range(block, self.tokens, self.body_len))
            .unwrap_or(0..self.body_len);
        ...
        let activation = binding_activation(rule, self.tokens, self.body, self.body_len);
        self.bindings.standard_paths.extend(
            introduced
                .into_iter()
                .filter(|binding| {
...
impl RustValidatedListener<io::Error> for BindingCollector<'_> {
    fn enter_macro_rules_definition(
        &mut self,
        context: &MacroRulesDefinitionContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_macro_definition(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_mod_decl(
        &mut self,
        context: &ModDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        let rule = context.as_rule_node();
        self.collect_standard_paths(
            rule,
            identifier_name(rule, self.tokens)
    ...
    fn enter_mod_decl_short(
        &mut self,
        context: &ModDeclShortContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        let rule = context.as_rule_node();
        self.collect_standard_paths(
            rule,
    ...
    fn enter_extern_crate(
        &mut self,
        context: &ExternCrateContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        let rule = context.as_rule_node();
        self.collect_standard_paths(
            rule,
            extern_crate_binding(rule, self.tokens)
                .into_iter()
                .collect(),
        );
        Ok(())
...
impl<'a> SyntaxAnalyzer<'a> {
    fn new(
        ...
        Self {
            ...
            analysis_root,
            bindings,
            syntax: RustSyntax::default(),
        }
    }

    fn finish(self) -> RustSyntax {
        self.syntax
    }

    fn collect_type_identifier(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        collect_direct_identifier_start(
            rule,
            self.tokens,
            self.body_len,
            &mut self.syntax.type_identifier_byte_starts,
        );
    }

    fn collect_declaration_identifier(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        collect_direct_identifier_start(
            rule,
            self.tokens,
            self.body_len,
            &mut self.syntax.declaration_identifier_byte_starts,
        );
    }

    fn collect_function(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        collect_function_binding(
            rule,
            self.tokens,
            self.body,
            self.body_len,
    ...
    fn collect_opaque_identifiers(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        collect_identifier_starts(
            rule.node(),
            self.tokens,
            self.body_len,
            &mut self.syntax.opaque_macro_identifier_byte_starts,
        );
    }

    fn collect_item_value(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        if item_introduces_unqualified_value(rule) {
            collect_value_binding(
                rule,
                self.tokens,
                self.body,
                self.body_len,
            ...
            self.collect_declaration_identifier(rule);
        }
    }

    fn collect_macro_invocation(&mut self, rule: antlr4_runtime::RuleNodeView<'_>) {
        collect_opaque_macro_invocation_identifiers(
            rule,
            self.tokens,
            self.body_len,
            &self.bindings.macros,
            &self.bindings.standard_paths,
            &mut self.syntax,
        );
    }
}

impl RustValidatedListener<io::Error> for SyntaxAnalyzer<'_> {
    fn enter_type_path_main(
        &mut self,
        context: &TypePathMainContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        collect_identifier_starts_excluding_rules(
            context.as_rule_node().node(),
            self.tokens,
            self.body_len,
            &[RULE_EXPR],
            &mut self.syntax.type_identifier_byte_starts,
    ...
    fn enter_type_parameter(
        &mut self,
        context: &TypeParameterContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        let rule = context.as_rule_node();
    ...
    fn enter_type_decl(
        &mut self,
        context: &TypeDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_type_identifier(context.as_rule_node());
        Ok(())
    }

    fn enter_enum_decl(
        &mut self,
        context: &EnumDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_type_identifier(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_union_decl(
        &mut self,
        context: &UnionDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_type_identifier(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_trait_decl(
        &mut self,
        context: &TraitDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_type_identifier(context.as_rule_node());
        Ok(())
    }

    fn enter_trait_alias(
        &mut self,
        context: &TraitAliasContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_type_identifier(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_macro_decl(
        &mut self,
        context: &MacroDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_type_identifier(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_mod_decl(
        ...
        let rule = context.as_rule_node();
        self.collect_type_identifier(rule);
        if let Some(range) = body_byte_range(rule, self.tokens, self.body_len) {
            self.syntax.inline_module_ranges.push(range);
        }
        Ok(())
    }

    fn enter_struct_decl(
        &mut self,
        context: &StructDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        let rule = context.as_rule_node();
        self.collect_type_identifier(rule);
    ...
    fn enter_enum_variant_main(
        &mut self,
        context: &EnumVariantMainContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_declaration_identifier(context.as_rule_node());
        Ok(())
    }

    fn enter_fn_decl(
        &mut self,
        context: &FnDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_function(context.as_rule_node());
        Ok(())
    }

    fn enter_method_decl(
        &mut self,
        context: &MethodDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_function(context.as_rule_node());
        Ok(())
    }

    fn enter_trait_method_decl(
        &mut self,
        context: &TraitMethodDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_function(context.as_rule_node());
        Ok(())
    }

    fn enter_foreign_fn_decl(
        &mut self,
        context: &ForeignFnDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_function(context.as_rule_node());
        Ok(())
    }

    fn enter_field(&mut self, context: &FieldContext<'_, ValidatedTreeContext>) -> io::Result<()> {
        let rule = context.as_rule_node();
        if rule.child_rule(RULE_FIELD_NAME).is_none() {
            collect_direct_identifier_start(
                rule,
                self.tokens,
                self.body_len,
                &mut self.syntax.struct_field_shorthand_byte_starts,
            );
        }
        Ok(())
    }
    ...
    fn enter_fn_head(
        &mut self,
        context: &FnHeadContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        let rule = context.as_rule_node();
        self.collect_declaration_identifier(rule);
        if fn_head_introduces_unqualified_value(rule, self.analysis_root) {
            collect_value_binding(
                rule,
                self.tokens,
                self.body,
    ...
    fn enter_static_decl(
        &mut self,
        context: &StaticDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_item_value(context.as_rule_node());
        Ok(())
    }

    fn enter_const_decl(
        &mut self,
        context: &ConstDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_item_value(context.as_rule_node());
        Ok(())
    }

    fn enter_associated_static_decl(
        &mut self,
        context: &AssociatedStaticDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_declaration_identifier(context.as_rule_node());
        Ok(())
    }

    fn enter_associated_const_decl(
        &mut self,
        context: &AssociatedConstDeclContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_declaration_identifier(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_trait_item(
        &mut self,
        context: &TraitItemContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_declaration_identifier(context.as_rule_node());
        Ok(())
    }

    fn enter_impl_item_tail(
        &mut self,
        context: &ImplItemTailContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_declaration_identifier(context.as_rule_node());
        Ok(())
    }

    fn enter_foreign_item_tail(
        &mut self,
        context: &ForeignItemTailContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        collect_foreign_item_binding(
    ...
    fn enter_prim_expr_no_struct(
        &mut self,
        context: &PrimExprNoStructContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        collect_closure_binding(
            context.as_rule_node(),
            self.tokens,
            self.body,
            self.body_len,
    ...
    fn enter_attr(&mut self, context: &AttrContext<'_, ValidatedTreeContext>) -> io::Result<()> {
        self.collect_opaque_identifiers(context.as_rule_node());
        Ok(())
    }

    fn enter_inner_attr(
        &mut self,
        context: &InnerAttrContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_opaque_identifiers(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_macro_tail(
        &mut self,
        context: &MacroTailContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
    ...
    fn enter_macro_invocation(
        &mut self,
        context: &MacroInvocationContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_macro_invocation(context.as_rule_node());
        Ok(())
    }

    fn enter_macro_invocation_semi(
        &mut self,
        context: &MacroInvocationSemiContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        self.collect_macro_invocation(context.as_rule_node());
        Ok(())
    }
    ...
    fn enter_lifetime(
        &mut self,
        context: &LifetimeContext<'_, ValidatedTreeContext>,
    ) -> io::Result<()> {
        collect_lifetime_identifier_start(
            context.as_rule_node(),
            self.tokens,
            self.body_len,
            &mut self.syntax,
