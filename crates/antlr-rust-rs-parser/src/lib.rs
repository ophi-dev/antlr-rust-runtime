//! Checked-in Rust recognizers and syntax queries for embedded-action lowering.
//!
//! This crate is a lockstep implementation dependency of
//! `antlr-rust-codegen`, not a standalone Rust parser API.
...
#[doc(hidden)]
pub use cfg_syntax::{cfg_all_predicate, member_cfg_predicates};
...
#[derive(Debug)]
pub struct ConditionalBindingFallback {
    pub insertion: usize,
    pub active_predicate: String,
}
...
#[derive(Debug)]
pub struct ScopedValueBinding {
    pub declaration_start: usize,
    pub scope: Range<usize>,
    pub cfg_fallback: Option<ConditionalBindingFallback>,
}

#[derive(Debug)]
pub struct ClosureBinding {
    pub parameter_ranges: Vec<Range<usize>>,
    pub cfg_parameter_predicates: Vec<(Range<usize>, String)>,
    pub scope: Range<usize>,
}
...
impl RustSyntax {
    pub fn is_type_identifier(&self, byte_start: usize) -> bool {
        self.type_identifier_byte_starts.contains(&byte_start)
    }

    pub fn is_declaration_identifier(&self, byte_start: usize) -> bool {
        self.declaration_identifier_byte_starts
            .contains(&byte_start)
    }

    pub fn is_non_value_identifier(&self, byte_start: usize) -> bool {
        self.non_value_identifier_byte_starts.contains(&byte_start)
    }

    pub fn is_opaque_macro_identifier(&self, byte_start: usize) -> bool {
        self.opaque_macro_identifier_byte_starts
            .contains(&byte_start)
    }

    pub fn is_opaque_macro_byte(&self, byte_start: usize) -> bool {
        self.opaque_macro_byte_ranges
            .iter()
            .any(|range| range.contains(&byte_start))
    }

    pub fn opaque_macro_accepts_expression_fallback(&self, byte_start: usize) -> bool {
        self.opaque_expression_macro_byte_ranges
            .iter()
            .any(|range| range.contains(&byte_start))
    }

    pub fn opaque_macro_requires_parent_block_fallback(&self, byte_start: usize) -> bool {
        self.opaque_parent_block_macro_byte_ranges
            .iter()
            .any(|range| range.contains(&byte_start))
    }

    pub fn conditional_macro_fallback(&self, byte_start: usize) -> Option<(usize, &str)> {
    ...
    pub fn is_struct_field_shorthand(&self, byte_start: usize) -> bool {
        self.struct_field_shorthand_byte_starts
            .contains(&byte_start)
    }

    pub fn is_pattern_field_shorthand(&self, byte_start: usize) -> bool {
    ...
    pub fn inline_module_depth(&self, byte_start: usize) -> usize {
    ...
    pub fn value_binding_byte_starts(&self) -> impl Iterator<Item = usize> + '_ {
        self.value_binding_byte_starts.iter().copied()
    }

    pub fn value_binding_cfg_fallback(
        &self,
        declaration_start: usize,
    ...
    pub fn scoped_value_bindings(&self) -> &[ScopedValueBinding] {
    ...
    pub fn closure_bindings(&self) -> &[ClosureBinding] {
        &self.closure_bindings
    }
}

pub fn analyze(body: &str) -> io::Result<RustSyntax> {
    analyze_with_root(body, AnalysisRoot::Body)
}

pub fn analyze_member_item(body: &str) -> io::Result<RustSyntax> {
...
fn collect_value_binding(
    ...
    let Some(active_predicate) = cfg_all_predicate(&member_cfg_predicates(
...
fn foreign_item_cfg_fallback(
    ...
    let active_predicate = cfg_all_predicate(&predicates)?;
...
fn cfg_predicates_before_child(
    ...
    member_cfg_predicates(&body[owner_range.start..child_range.start])
}
...
fn collect_const_generic_binding(
    ...
    let cfg_fallback = body_byte_range(parameter, tokens, body_len)
        .and_then(|range| {
            cfg_all_predicate(&member_cfg_predicates(
...
fn binding_activation(
    ...
    let Some(predicate) = cfg_all_predicate(&member_cfg_predicates(
...
fn collect_pattern_field_roles(
    ...
    for node in root.descendants() {
        ...
        let Some(active_predicate) =
            cfg_all_predicate(&member_cfg_predicates(&body[range.clone()]))
        else {
            continue;
        };
        syntax
...
pub fn struct_has_value_constructor(item: &str) -> io::Result<bool> {
