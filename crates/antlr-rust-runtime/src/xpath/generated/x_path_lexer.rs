...
/// `let vocabulary = metadata().vocabulary(); for token in lex(src, XPathLexer::new).tokens() { println!("type={} channel={} text={:?}", vocabulary.display_name(token.token_type()), token.channel(), token.text()); }`
///
