...
fn render_parser_with_options(
    ...
    let parser_rustdoc = render_parser_rustdoc(data, &entry_rule_indices);
...
fn render_parser_rustdoc(data: &InterpData, entry_rule_indices: &[usize]) -> String {
    let all_method_capacity = data
        .rule_names
        .iter()
        .map(|rule| rule.len() + "/// - `()`\n".len())
        .sum::<usize>();
    let entry_method_capacity = entry_rule_indices
        .iter()
        .filter_map(|index| data.rule_names.get(*index))
        .map(|rule| rule.len() + "/// - `()`\n".len())
        .sum::<usize>();
    ...
    if !entry_rule_indices.is_empty() {
        ...
        for index in entry_rule_indices {
            let Some(rule) = data.rule_names.get(*index) else {
                continue;
            };
...
"#,
    ...
    fn renders_parser_rustdoc_with_entry_rule_methods() {
        ...
        let rendered = render_parser_rustdoc(&data, &entry_rule_indices);
